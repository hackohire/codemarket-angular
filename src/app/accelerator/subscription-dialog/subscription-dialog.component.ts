...
export class SubscriptionDialogComponent implements OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    ...
    if (!this.authService.loggedInUser.stripeId) {
      const updatedUser = await this.acceleratorService.createStripeUser();
  ...
  async handleForm() {
    ...
    try {
      ...
      const successfulSubscription = await this.acceleratorService.attachCardAndCreateSubscription(
        token.id,
        this.stripeId,
  ...
  async applyPromo() {
    if (this.promoCodeControl.value === this.promoCodes.FIRSTMONTHFREE.title) {
      ...
      const coupon = await this.acceleratorService.getCouponByName(this.promoCodeControl.value);
      console.log(coupon);
      if (coupon) {
