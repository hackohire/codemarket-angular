...
@Component({
  selector: 'app-add-customer-template',
  templateUrl: './add-customer-template.component.html',
  styleUrls: ['./add-customer-template.component.scss']
})
export class AddCustomerTemplateComponent implements OnInit {
  breadcumb: BreadCumb;
  postForm: FormGroup;
  ...
  constructor(
    ...
    this.breadcumb = {
      title: 'Add Customer Template Details',
      path: [

        {
          name: CompanyPostTypes.CustomerTemplate
        }
      ]
    };

     /** Make the Changes here while creating new post type */
    if (this.activatedRoute.snapshot.parent.routeConfig.path === `add-${CompanyPostTypes.CustomerTemplate}`) {
  ...
  postFormInitialization(i: Post) {
    this.postForm = new FormGroup({
      ...
      // change this line
      type: new FormControl(CompanyPostTypes.CustomerTemplate),
    });
  }
