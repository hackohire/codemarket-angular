...
@Component({
  selector: 'app-add-marketing-goal',
  templateUrl: './add-marketing-goal.component.html',
  styleUrls: ['./add-marketing-goal.component.scss']
})
export class AddMarketingGoalComponent implements OnInit {
  breadcumb: BreadCumb;
  postForm: FormGroup;
  ...
  constructor(
    ...
    this.breadcumb = {
      title: 'Add Marketing Goal Details',
      path: [

        {
          name: CompanyPostTypes.MarketingGoal
        }
      ]
    };

     /** Make the Changes here while creating new post type */
    if (this.activatedRoute.snapshot.parent.routeConfig.path === `add-${CompanyPostTypes.MarketingGoal}`) {
  ...
  postFormInitialization(i: Post) {
    this.postForm = new FormGroup({
      ...
      // change this line
      type: new FormControl(CompanyPostTypes.MarketingGoal),
    });
  }
