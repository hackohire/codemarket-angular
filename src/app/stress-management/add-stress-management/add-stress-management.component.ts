...
@Component({
  selector: 'app-add-stress-management',
  templateUrl: './add-stress-management.component.html',
  styleUrls: ['./add-stress-management.component.scss']
})
export class AddStressManagementComponent implements OnInit {
  breadcumb: BreadCumb;
  postForm: FormGroup;
  ...
  constructor(
    ...
    this.breadcumb = {
      title: 'Add Stress Management Details',
      path: [

        {
          name: UserProfilePostTypes.StressManagement
        }
      ]
    };

     /** Make the Changes here while creating new post type */
    if (this.activatedRoute.snapshot.parent.routeConfig.path === `add-${UserProfilePostTypes.StressManagement}`) {
  ...
  postFormInitialization(i: Post) {
    this.postForm = new FormGroup({
      ...
      // change this line
      type: new FormControl(UserProfilePostTypes.StressManagement),
    });
  }
