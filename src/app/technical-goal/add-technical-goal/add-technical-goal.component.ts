...
@Component({
  selector: 'app-add-technical-goal',
  templateUrl: './add-technical-goal.component.html',
  styleUrls: ['./add-technical-goal.component.scss']
})
export class AddTechnicalGoalComponent implements OnInit {
  breadcumb: BreadCumb;
  postForm: FormGroup;
  ...
  constructor(
    ...
    this.breadcumb = {
      title: 'Add Technical Goal Details',
      path: [

        {
          name: CompanyPostTypes.TechnicalGoal
        }
      ]
    };

     /** Make the Changes here while creating new post type */
    if (this.activatedRoute.snapshot.parent.routeConfig.path === `add-${CompanyPostTypes.TechnicalGoal}`) {
  ...
  postFormInitialization(i: Post) {
    this.postForm = new FormGroup({
      ...
      // change this line
      type: new FormControl(CompanyPostTypes.TechnicalGoal),
    });
  }
