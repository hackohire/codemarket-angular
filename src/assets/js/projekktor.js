        ...
        function PPlayer(srcNode, cfg, onReady) {
            ...
            this._enterFullViewport = function(forcePlayer) {
                ...
                target.data("fsdata", {
                    ...
                    bodyOverflowX: $(win[0].document.body).css("overflow-x"),
                    bodyOverflowY: $(win[0].document.body).css("overflow-y"),
            ...
            this._exitFullViewport = function(forcePlayer) {
                ...
                if (fsData != null) {
                    ...
                    if (fsData.iframeWidth > 0 && !forcePlayer) {
                        target.attr("width", fsData.iframeWidth + "px").attr("height", fsData.iframeHeight + "px")
                    } else {
                        target.width(fsData.targetWidth).height(fsData.targetHeight)
                    }
                    target.attr("style", (fsData.targetStyle == null) ? "" : fsData.targetStyle).data("fsdata", null)
                }
            };
            ...
            this.setSize = function(data) {
                var target = this.getIframe() || this.getDC(),
                    fsdata = target.data("fsdata") || null,
                    w = (data && data.width != null) ? data.width : (this.getConfig("width") != null) ? this.getConfig("width") : false,
                    h = (data && data.height != null) ? data.height : (this.getConfig("height") == null && this.getConfig("ratio")) ? Math.round((w || this.getDC().width()) / this.getConfig("ratio")) : (this.getConfig("height") != null) ? this.getConfig("height") : false;
...
jQuery(function(a) {
    $p.utils = {
        ...
        embeddFlash: function(e, d, l, c) {
            ...
            for (var k in h) {
                if (typeof h[k] != "function") {
                    g = h[k];
                    d.src += k + "=" + encodeURIComponent(g) + "&"
                }
            }
            d.src.replace(/&$/, "");
            f = '<object id="' + d.id + '" codebase="https://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0"  name="' + d.name + '" width="' + d.width + '" height="' + d.height + '" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"><param name="movie" value="' + d.src + '"></param><param name="allowScriptAccess" value="' + d.allowScriptAccess + '"></param><param name="allowFullScreen" value="' + d.allowFullScreen + '"></param><param name="wmode" value="' + d.wmode + '"></param>';
            m = "<embed ";
            for (var k in d) {
                if (k.toUpperCase() === "FLASHVARS") {
                    continue
                }
            ...
            m += ' pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"></embed>';
            n = f + m;
            n += "</object>";
            ...
            j.get(0).innerHTML = n;
            if (l !== false) {
                j.append(a("<div/>").attr("id", d.id + "_cc").css({
                    width: (c) ? "1px" : "100%",
                    height: (c) ? "1px" : "100%",
        ...
        cssTransitions: function() {
            ...
            function e(j) {
                var h = "Webkit Moz O ms Khtml".split(" "),
                    f = j.charAt(0).toUpperCase() + j.substr(1),
                    g = (j + " " + h.join(f + " ") + f).split(" ");
                return c(g)
            }
            return e("animationName")
        },
        versionCompare: function(f, g) {
            var d = f.split("."),
                c = g.split(".");
    ...
    $p.platforms = {
        VLC: function() {
            if (navigator.plugins && (navigator.plugins.length > 0)) {
                for (var b = 0; b < navigator.plugins.length; ++b) {
                    if (navigator.plugins[b].name.indexOf("VLC") != -1) {
                        if (navigator.plugins[b].version != null) {
                            return navigator.plugins[b].version || "0"
                        }
                        if (navigator.plugins[b].description != null) {
                            if (navigator.plugins[b].description.match(/\d{1,}\.\d{1,}\.\d{1,}/i)[0]) {
                                return navigator.plugins[b].description.match(/\d{1,}\.\d{1,}\.\d{1,}/i)[0]
                            }
                        }
                    }
                }
        ...
        IOS: function(c) {
            var b = navigator.userAgent.toLowerCase(),
                d = b.indexOf("os ");
            if ((b.indexOf("iphone") > -1 || b.indexOf("ipad") > -1) && d > -1) {
                return (b.substr(d + 3, 3).replace("_", ".")).toString()
            }
            return "0"
        },
        NATIVE: function(c) {
            try {
                var b = a((c.indexOf("video") > -1) ? "<video/>" : "<audio/>").get(0);
                if (b.canPlayType != null) {
                    if (c === "*") {
                        return "1"
                    }
...
jQuery(function($) {
    projekktorPluginInterface.prototype = {
        ...
        cookie: function(key, value, ttl) {
            ...
            if (this.pp.getConfig("cookieExpiry") == 0) {
                return null
            }
            var t = new Date(),
                result = null,
            ...
            cookieString = encodeURIComponent(this.getConfig("cookieName", "projekktor") + "_" + this.name) + "=" + encodeURIComponent(storedData) + "; expires=" + ((ttl == false) ? "Thu, 01 Jan 1970 00:00:01 GMT" : t.toUTCString()) + "; path=" + this.getConfig("cookiePath", "/");
...
jQuery(function(a) {
    $p.newModel({
        ...
        setiLove: function() {
            ...
            if (navigator.plugins && (navigator.plugins.length > 0)) {
                for (var d = 0; d < navigator.plugins.length; ++d) {
                    if (navigator.plugins[d].name.indexOf("VLC") > -1) {
                        for (var b = 0; b < navigator.plugins[d].length; b++) {
                            var e = navigator.plugins[d][b];
                            if (e.suffixes != null && e.type != null) {
                                a.each(e.suffixes.split(","), function(f, g) {
                                    c.iLove.push({
                                        ext: g,
                                        type: e.type.replace(/x-/, ""),
                                        platform: ["vlc"],
        ...
        applyMedia: function(b) {
            ...
            this.mediaElement = a("#" + this.pp.getMediaId() + "_vlc");
            this.applySrc()
        },
        applySrc: function() {
            var c = this,
                b = this.getSource();
            this.mediaElement.get(0).playlist.add(b[0].src, "item 1");
            if (this.getState("PLAYING")) {
                this.setPlay();
        ...
        addListeners: function() {
            ...
            a.each(this._eventMap, function(c, d) {
                try {
                    if (b.mediaElement.get(0).attachEvent) {
                        b.mediaElement.get(0).attachEvent(c, function(e) {
                            b[d](this, e)
                        })
                    } else {
                        if (b.mediaElement.get(0).addEventListener) {
                            b.mediaElement.get(0).addEventListener(c, function(e) {
                                b[d](this, e)
                            }, false)
                        } else {
                            b.mediaElement.get(0)["on" + c] = function(e) {
                                b[d](this, e)
                            }
                        }
                    }
        ...
        setVolume: function(b) {
            this._volume = b;
            this.mediaElement.get(0).audio.volume = b * 100;
            this.volumeListener(b)
        },
        setSeek: function(b) {
            this.mediaElement.get(0).input.position = b / this.media.duration;
            this._setSeekState("seeked", b)
        },
...
jQuery(function(a) {
    $p.newModel({
        ...
        iLove: [{
            ...
            type: "video/flv",
            platform: "flash",
            streamType: ["http", "httpVideo", "pseudo", "rtmp"],
            fixed: true
        }, {
            ext: "mp4",
            type: "video/mp4",
        ...
        applySrc: function() {
            var c = this,
                b = this.getSource();
            this.mediaElement.get(0).api_source(b[0].src);
            this.seekedListener();
            if (this.getState("PLAYING")) {
                this.setPlay();
                if (c.isPseudoStream !== true) {
        ...
        addListeners: function() {
            ...
            var b = this;
            a.each(this._eventMap, function(c, d) {
                b.mediaElement.get(0).api_addlistener(c, "projekktor('" + b.pp.getId() + "').playerModel." + d)
            })
        },
        removeListeners: function() {
            try {
                this.mediaElement.get(0).api_removelistener("*")
            } catch (b) {}
        },
        ...
        metaDataListener: function(c) {
            ...
            try {
                this.mediaElement.get(0).api_seek(this.media.position || 0)
            } catch (b) {}
        ...
        setSeek: function(c) {
            if (this.isPseudoStream) {
                ...
                try {
                    this.mediaElement.get(0).api_seek(c)
                } catch (b) {}
                this.seekedListener();
                this.timeListener({
                    position: c
        ...
        setVolume: function(b) {
            this._volume = b;
            try {
                this.mediaElement.get(0).api_volume(b)
            } catch (c) {
                return false
            }
            return b
        ...
        setPause: function(b) {
            try {
                this.mediaElement.get(0).api_pause()
            } catch (c) {}
        },
        setPlay: function(b) {
            try {
                this.mediaElement.get(0).api_play()
            } catch (c) {}
        },
        getVolume: function() {
        ...
        detachMedia: function() {
            this.setPause();
            try {
                a(this.mediaElement).get(0).remove()
            } catch (b) {}
        }
    });
    $p.newModel({
...
jQuery(function(a) {
    $p.newModel({
        ...
        init: function() {
            ...
            if (b.indexOf("Android") >= 0) {
                if (parseFloat(b.slice(b.indexOf("Android") + 8)) < 3) {
        ...
        applySrc: function() {
            ...
            a.each(this.getSource(), function() {
                ...
                e.appendTo(d.mediaElement)
            });
            var c = function() {
                    d.mediaElement.unbind("loadstart.projekktorqs" + d.pp.getId());
                    d.mediaElement.unbind("loadeddata.projekktorqs" + d.pp.getId());
                    d.mediaElement.unbind("canplay.projekktorqs" + d.pp.getId());
                    d.addListeners("error");
                    d.addListeners("play");
                    d.addListeners("loadstart");
                    d.addListeners("canplay");
                    d.mediaElement = a("#" + d.pp.getMediaId() + "_html");
                    if (b) {
                        d.displayReady();
                        return
                    }
                    if (d.getSeekState("SEEKING")) {
                        if (d._isPlaying) {
                            d.setPlay()
                        }
                        d.seekedListener();
        ...
        playingListener: function(c) {
            ...
            if (!this.isGingerbread) {
                (function() {
                    try {
                        if (b.getDuration() === 0) {
                            if (b.mediaElement.get(0).currentSrc !== "" && b.mediaElement.get(0).networkState == b.mediaElement.get(0).NETWORK_NO_SOURCE) {
                                b.sendUpdate("error", 80);
                                return
                            }
    ...
    $p.newModel({
        ...
        applyMedia: function(b) {
            ...
            var c = a("#" + this.pp.getMediaId() + "_audio_container");
            if (c.length === 0) {
                c = a("<div/>").css({
                    width: "1px",
                    height: "1px",
            ...
            c.html("").append(a((this.isGingerbread) ? "<video/>" : "<audio/>").attr({
                id: this.pp.getMediaId() + "_html",
...
jQuery(function(a) {
    playerModel.prototype = {
        ...
        getBufferState: function(c) {
            var b = (this._currentBufferState == null) ? "NONE" : this._currentBufferState;
            if (c != null) {
                return (b == c.toUpperCase())
            }
            return b
        },
        getSeekState: function(c) {
            var b = (this._currentSeekState == null) ? "NONE" : this._currentSeekState;
            if (c != null) {
                return (b == c.toUpperCase())
            }
            return b
        ...
        getPoster: function() {
            var b = this.pp.getConfig("poster"),
                e = "default",
                f = [];
            if (typeof b != "object") {
                return b
            ...
            e = this.pp._getAppropriateQuality(f);
            for (var c in this.pp.getConfig("poster")) {
                if (this.pp.getConfig("poster")[c].quality == e) {
                    b = this.pp.getConfig("poster")[c].src || null;
                    break
                }
            }
            return b
        ...
        getSource: function() {
            var c = [],
                e = this.media.offset || this.media.position || false,
                d = this,
        ...
        timeListener: function(d) {
            ...
            if (b == this.media.position) {
                return
            }
            if (this.isPseudoStream && Math.round(b * 100) / 100 == Math.round(this.media.offset * 100) / 100) {
        ...
        loadProgressUpdate: function() {
            var c = this.mediaElement.get(0),
                b = 0;
            if (typeof c.buffered !== "object") {
                return
            }
            if (c.buffered.length === 0 && c.seekable.length === 0) {
                return
            }
            if (this.media.loadProgress == 100) {
                return
            }
            if (c.seekable && c.seekable.length > 0) {
                b = Math.round(c.seekable.end(0) * 100 / this.media.duration)
            } else {
                b = Math.round(c.buffered.end(c.buffered.length - 1) * 100) / this.media.duration
            }
        ...
        progressListener: function(h, c) {
            if (this.mediaElement instanceof jQuery) {
            ...
            var b = (g > 0 && d > 0) ? g * 100 / d : 0;
            if (Math.round(b) > Math.round(this.media.loadProgress)) {
                this._KbPerSec = ((g / 1024) / (((new Date()).getTime() - this._bandWidthTimer) / 1000))
            }
            b = (this.media.loadProgress !== 100) ? b : 100;
            b = (this.allowRandomSeek === true) ? 100 : 5 * Math.round(b / 5);
            if (this.media.loadProgress != b) {
        ...
        metaDataListener: function(c) {
            try {
                this.media.videoWidth = c.videoWidth;
                this.media.videoHeight = c.videoHeight
            } catch (b) {}
            this._scaleVideo()
        },
        setTestcard: function(f, b) {
            var e = this.pp.getMediaContainer().html("").css({
                width: "100%",
                height: "100%"
            }),
                d = this.pp.getConfig("messages"),
                c = (b !== undefined && b !== "") ? b : (d[f] !== undefined) ? d[f] : d[0];
            this.removeListeners();
            this.detachMedia();
        ...
        applyImage: function(d, b) {
            var f = a("<img/>").hide(),
                e = this;
            $p.utils.blockSelection(f);
            if (d === "" || d === undefined) {
                return a("<span/>").attr({
                    id: this.pp.getMediaId() + "_image"
                }).appendTo(b)
            }
            f.html("").appendTo(b).attr({
                id: this.pp.getMediaId() + "_image",
            ...
            f.load(function(h) {
                ...
                f.show();
                $p.utils.stretch(e.pp.getConfig("imageScaling"), f, b.width(), b.height())
            });
            f.attr("src", d);
            var c = function(i, g) {
                    if (g.is(":visible") === false) {
                    ...
                    var h = g.width(),
                        ...
                        k = i.width(),
                        m = i.height();
                    if ($p.utils.stretch(e.pp.getConfig("imageScaling"), i, g.width(), g.height())) {
                        try {
                            e.sendUpdate("scaled", {
                                realWidth: i._originalDimensions.width,
                                realHeight: i._originalDimensions.height,
                                displayWidth: e.mediaElement.width(),
                                displayHeight: e.mediaElement.height()
                            })
                        } catch (l) {}
                    }
                    if (i.attr("src") != e.getPoster()) {
                        i.attr("src", e.getPoster())
                    }
                };
        ...
        createFlash: function(d, b, c) {
            this.mediaElement = $p.utils.embeddFlash(b.html(""), d, c, true);
            this._waitforPlayer()
        },
            ...
            (function() {
                ...
                try {
                    if (d === undefined) {
                        ...
                        if (d.get(0)[c.flashVerifyMethod] === undefined) {
                            setTimeout(arguments.callee, 200)
                        } else {
                            c._setBufferState("full");
                            c.flashReadyListener();
                            a("#" + a(c.mediaElement).attr("id") + "_cc").css({
                                width: "100%",
                                height: "100%"
                            })
                        }
        ...
        _setState: function(c) {
            var b = this;
            c = c.toUpperCase();
            if (this._currentState != c && this._currentState != "ERROR") {
        ...
        _setSeekState: function(c, b) {
            if (this._currentSeekState != c.toUpperCase()) {
                this._currentSeekState = c.toUpperCase();
        ...
        _scaleVideo: function(h) {
            ...
            try {
                var f = d.width(),
                    i = d.height(),
                    b = this.media.videoWidth,
                    c = this.media.videoHeight;
...
jQuery(function(a) {
    projekktorDisplay.prototype = {
        ...
        initialize: function() {
            ...
            this.imaContainer = this.applyToPlayer(a("<div/>").addClass("ima"), "ima");
            this.setActive();
    ...
    projekktorControlbar.prototype = {
        ...
        initialize: function() {
            ...
            for (var d in this.controlElementsConfig) {
                ...
                $p.utils.blockSelection(a(this.controlElements[d]))
            }
        ...
        addGuiListeners: function() {
            ...
            a.each(this.controlElementsConfig, function(c, e) {
                ...
                for (var d = 0; d < e.length; d++) {
                    ...
                    a.each(e[d].on, function(i, f) {
                        var g = ("on" + f in window.document),
                            j = e[d].call;
                        if (!g) {
                            var h = document.createElement("div");
                            h.setAttribute("on" + f, "return;");
                            g = (typeof h["on" + f] == "function")
                        }
                        if (g) {
                            b.controlElements[c].bind(f, function(k) {
                                b.clickCatcher(k, j, b.controlElements[c])
                            })
                        }
                    })
                }
            ...
            this.cb.mouseout(function(c) {
                b.controlsBlur(c)
            })
        },
        clickCatcher: function(b, e, c) {
            var d = this;
            b.stopPropagation();
            b.preventDefault();
            this[e](b, c);
        ...
        displayTime: function(f, d, i) {
            if (this.pp.getHasGUI()) {
                return
            }
            var e = Math.round((f || this.pp.getLoadPlaybackProgress() || 0) * 10) / 10,
                g = d || this.pp.getDuration() || 0,
                b = i || this.pp.getPosition() || 0,
                h = a.extend({}, this._clockDigits(g, "dur"), this._clockDigits(b, "elp"), this._clockDigits(g - b, "rem"));
            if (this.controlElements.playhead.data("pct") != e) {
                this.controlElements.playhead.data("pct", e).css({
                    width: e + "%"
                });
                ...
                for (var c in this.controlElements) {
                    ...
                    if (h[c]) {
                        a.each(this.controlElements[c], function() {
                            a(this).html(h[c])
                        })
                    }
                }
        ...
        displayProgress: function() {
            var b = Math.round(this.pp.getLoadProgress() * 10) / 10;
            if (this.controlElements.loaded.data("pct") != b) {
                this.controlElements.loaded.data("pct", b).css("width", b + "%")
            }
        },
        displayVolume: function(h) {
            var g = this;
            ...
            var b = this.cb.hasClass("active"),
                g = this,
                f = this.getConfig("fixedVolume"),
                e = (this.controlElements.mute.hasClass("toggle") || this.controlElements.unmute.hasClass("toggle") || this.getConfig("toggleMute"));
            this._active("mute", !f);
            this._active("unmute", !f);
            this._active("vmax", !f);
            this._active("vknob", !f);
            this._active("vmarker", !f);
            this._active("vslider", !f);
            if (f) {
                return
            }
            if (this.controlElements.vslider.width() > this.controlElements.vslider.height()) {
                this.controlElements.vmarker.css("width", h * 100 + "%");
                this.controlElements.vknob.css("left", h * 100 + "%")
            } else {
                this.controlElements.vmarker.css("height", h * 100 + "%");
                this.controlElements.vknob.css("bottom", h * 100 + "%")
            }
            var c = this.controlElements.volume.find("li"),
                j = c.length - Math.ceil((h * 100) / c.length);
            for (var d = 0; d <= c.length; d++) {
                if (d >= j) {
                    a(c[d]).addClass("active")
                } else {
        ...
        volumeBtnHover: function(b) {
            clearTimeout(this._outDelay);
            this.setActive(this.controlElements.volumePanel, true)
        },
        volumeBtnOut: function(b, d) {
            var c = this;
            if (b.currentTarget != d.get(0)) {
                return
            }
            if (b.relatedTarget == d.get(0)) {
                return
            }
            this._outDelay = setTimeout(function() {
                c.setActive(c.controlElements.volumePanel, false)
            }, 100)
        },
        vsliderClk: function(d) {
            if (this._vSliderAct == true) {
                return
            }
            var h = a(this.controlElements.vslider),
                e = h.width() > h.height() ? "hor" : "vert",
                c = (e == "hor") ? h.width() : h.height(),
                g = (d.originalEvent.touches) ? d.originalEvent.touches[0].pageX : d.pageX,
                f = (d.originalEvent.touches) ? d.originalEvent.touches[0].pageY : d.pageY,
                i = (e == "hor") ? g - h.offset().left : f - h.offset().top,
                b = 0;
            if (i < 0 || isNaN(i) || i == undefined) {
                b = 0
            } else {
                b = (e == "hor") ? (i / c) : 1 - (i / c)
            }
        ...
        scrubberShowTooltip: function(b) {
        ...
        scrubberHideTooltip: function(b) {
            this.setActive(this.controlElements.scrubbertip, false)
        },
        scrubberdragTooltip: function(j) {
            if (this.pp.getDuration() == 0) {
                return
            }
            this.setActive(this.controlElements.scrubbertip, true);
            var c = a(this.controlElements.scrubberdrag[0]),
                f = a(this.controlElements.loaded[0]),
                h = a(this.controlElements.scrubbertip),
                e = (j.originalEvent.touches) ? j.originalEvent.touches[0].pageX : j.pageX,
                d = (j.originalEvent.touches) ? j.originalEvent.touches[0].pageY : j.pageY,
                g = e - c.offset().left - (h.outerWidth() / 2),
                b = this._clockDigits(this.pp.getDuration() / 100 * ((e - c.offset().left) * 100 / c.width()), "tip");
            for (var i in this.controlElements) {
                if (i == "cb") {
                    break
                }
                if (b[i]) {
                    a.each(this.controlElements[i], function() {
                        a(this).html(b[i])
                    })
                }
            }
            g = (g < 0) ? 0 : g;
            g = (g > c.width() - h.outerWidth()) ? c.width() - h.outerWidth() : g;
            h.css({
                left: g + "px"
            })
        },
        scrubberdragStartDragListener: function(b) {
            ...
            var e = this,
                c = a(this.controlElements.scrubberdrag[0]),
                f = a(this.controlElements.loaded[0]),
                d = 0,
                j = Math.abs(parseInt(c.offset().left) - b.clientX),
                h = function(l) {
                    var k = Math.abs(c.offset().left - l.clientX);
                    k = (k > c.width()) ? c.width() : k;
                    k = (k > f.width()) ? f.width() : k;
                    k = (k < 0) ? 0 : k;
                    k = Math.abs(k / c.width()) * e.pp.getDuration();
                    if (k > 0 && k != d) {
                        d = k;
                        e.pp.setPlayhead(d)
                    }
            ...
            this.playerDom.bind("mouseup.slider", g);
            c.mouseup(g);
            c.mousemove(i);
            h(b)
        },
        ...
        _getVolume: function() {
            var c = parseFloat(this.cookie("volume") || this.getConfig("volume") || 0.5),
                b = this.cookie("muted") || false;
            if (this.getConfig("fixedVolume") || c == null) {
        ...
        _clockDigits: function(d, h) {
            if (d < 0 || isNaN(d) || d == undefined) {
                d = 0
            }
            var g = Math.floor(d / (60 * 60)),
                c = d % (60 * 60),
                b = Math.floor(c / 60),
                k = g * 60 + b,
                i = c % 60,
                ...
                j = {};
            j["min_" + h] = (b < 10) ? "0" + b : b;
            j["min_abs_" + h] = (k < 10) ? "0" + k : k;
            j["sec_" + h] = (e < 10) ? "0" + e : e;
            j["sec_abs_" + h] = (f < 10) ? "0" + f : f;
            j["hr_" + h] = (g < 10) ? "0" + g : g;
            return j
        }
    }
