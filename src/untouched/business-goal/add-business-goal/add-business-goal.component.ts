...
@Component({
  selector: 'app-add-business-goal',
  templateUrl: './add-business-goal.component.html',
  styleUrls: ['./add-business-goal.component.scss']
})
export class AddBusinessGoalComponent implements OnInit {
  breadcumb: BreadCumb;
  postForm: FormGroup;
  ...
  constructor(
    ...
    this.breadcumb = {
      title: 'Add Business Goal Details',
      path: [

        {
          name: CompanyPostTypes.BusinessGoal
        }
      ]
    };

     /** Make the Changes here while creating new post type */
    if (this.activatedRoute.snapshot.parent.routeConfig.path === `add-${CompanyPostTypes.BusinessGoal}`) {
  ...
  postFormInitialization(i: Post) {
    this.postForm = new FormGroup({
      ...
      // change this line
      type: new FormControl(CompanyPostTypes.BusinessGoal),
    });
  }
